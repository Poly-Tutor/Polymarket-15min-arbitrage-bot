...
impl MarketMonitor {
    ...
    async fn refresh_market_tokens(&self) -> Result<()> {
        ...
        // Get market details
        if let Ok(btc_15m_details) = self.api.get_market(&market_id).await {
    ...
    pub async fn fetch_market_data(&self) -> Result<MarketSnapshot> {
        ...
        let btc_15m_timestamp = Self::extract_timestamp_from_slug(&btc_15m_slug);
        ...
        let (btc_15m_up_price, btc_15m_down_price) = if btc_15m_remaining > 0 {
            tokio::join!(
        ...
        let message = format!(
            ...
            self.market_name, btc_15m_up_str, btc_15m_down_str, btc_15m_remaining_str, btc_15m_timestamp
        );
